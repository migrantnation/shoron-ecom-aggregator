@if($order_info)
        ...
        <div class="alert alert-danger fade in alert-dismissible text-center">
            ...
            @forelse(@$tracking_details as $each_tracking)
                @if($each_tracking->status == 5)
                    ...
                    {{@$each_tracking->message_by?' by ' .@$each_tracking->message_by:""}}<br>
                    <strong>Reason:</strong> {{$each_tracking->message}}<br>
    ...
    <form action="javascript:void(0)" method="get" id="SubmitTrackingMessage">
        {{csrf_field()}}
        <input type="hidden" name="order_code" id="order_code" value="{{$order_info->order_code}}">
        <div class="row">
            <div class="col-sm-6">
                <input type="text" class="form-control" required
                       name="message" id="message" maxlength="100"
                       placeholder="Enter you tracking message...">
            </div>
            <div class="col-sm-6">
                <button class="btn btn-info" type="submit">Add Message</button>
            </div>
        </div>
    </form>
    <hr>
...
<script>
    ...
    document.getElementById("SubmitTrackingMessage").onsubmit = function () {
        ...
        $.ajax({
            url: "{{url('lp/save-order-tracking-message')}}",
            type: "post",
            dataType: 'json',
            data: data,
            success: function (response) {
                if (response['meta']['status'] == 200) {
                    $('#tracking-info').html(response['response']['view']);
