...
@section('content')
    ...
    <div class="page-bar">
        <ul class="page-breadcrumb">
            ...
            <li>
                <span>{{ __('admin_text.edit-customer') }}</span>
            </li>
        </ul>
    </div>
    <!-- END PAGE BAR -->
    ...
    <div class="container-alt margin-top-20">
        ...
        <h3 class="margin-top-10 margin-bottom-15">{{ __('admin_text.edit-customer') }}</h3>
        ...
        <div class="portlet light bordered">
            ...
            <div class="portlet-body">
                <form action="{{route('udc.updateCustomer',$customer_info->id)}}" method="post">
                    {{csrf_field()}}
                    <div class="row">
                        <div class="col-sm-12">
                            <div class="form-group">
                                ...
                                <input type="text" class="form-control" name="customer_name"
                                       value="{{old('customer_name')?old('customer_name'):$customer_info->customer_name}}">
                            </div>
                            <div class="form-group">
                                <label for="">{{ __('admin_text.address') }}</label>
                                <input type="text" class="form-control" name="customer_address"
                                       value="{{old('customer_address')?old('customer_address'):$customer_info->customer_address}}">
                            </div>
                            <div class="form-group">
                                <label for="">{{ __('admin_text.mobile-number') }}</label>
                                <input type="text" class="form-control" name="customer_contact_number"
                                       value="{{old('customer_contact_number')?old('customer_contact_number'):$customer_info->customer_contact_number}}">
                            </div>
                            <hr>
