@if($order_info)
    ...
    <div class="tracking-details">
        {{--<div class="row"><p>No records found</p></div>--}}
        <div class="row">
        ...
        @if($order_info->status != 5)
            ...
            <div class="col-sm-3">
                <div class="tracking-status-box {{@$order_info->status == 2 ? 'active':(@$order_info->status > 2 ? 'completed':'')}}"
                    ...
                    @else
                        <div class="date">{{@$status_tracking_info[2][0]->created_at ? date("F d, Y", strtotime(@$status_tracking_info[2][0]->created_at)):''}}</div>
                        <div class="text">
                    ...
                    <div class="status-inner complete-stage">
                        <div class="status-text">
                            ...
                            <span class="tracking-date">
                                @if(@$status_tracking_info[5][0]->created_at)
                                    {{date('M d, Y h:i a', strtotime(@$status_tracking_info[5][0]->created_at))}}
                                @else
                                    No track. <br>Order Updated:
                                    {{date('M d, Y h:i a', strtotime($order_info->updated_at))}}
                                @endif
