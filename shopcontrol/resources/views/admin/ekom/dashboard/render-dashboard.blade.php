                ...
                <div class="m-widget14">
                    ...
                    <div class="row  align-items-center">
                        <div class="col">
                            ...
                            <?php
                                $register_user_kpi = 3000;
                                $register_user_percentage = (@$registered_users->count()/3000) * 100;
                                $connected_user_kpi = 2000;
                                $connected_user_percentage = (@$registered_users->count()/2000) * 100;
                                $transacting_user_kpi = 1500;
                                $transacting_user_percentage = (@$registered_users->count()/1500) * 100;
                                $non_transacting_user_kpi = 500;
                                $non_transacting_user_percentage = (@$registered_users->count()/500) * 100;
                                $outofreach_user_kpi = 1000;
                                $outofreach_user_percentage = (@$registered_users->count()/1000) * 100;
                            ?>

                            <div class="row" style="line-height: 1.8; margin-bottom: 20px;">
                                <div class="col-sm-6">Registered Users KPI: {{@$register_user_kpi}}</div>
                                <div class="col-sm-6">Connected Users KPI: {{@$connected_user_kpi}}</div>
                                <div class="col-sm-6">Transactions KPI: {{@$transacting_user_kpi}}</div>
                                <div class="col-sm-6">Non Transactions KPI: {{@$non_transacting_user_kpi}}</div>
                                <div class="col-sm-6">Out of Reach KPI: {{@$outofreach_user_kpi}}</div>
                            </div>
                            <div>
                                <canvas id="m_userStatistics" height="250"
                                        data-colors='["#6f42c1", "#34bfa3", "#ffb822", "#ffb822", "#f4516c"]'
                                        data-labels='["Registered", "Connected", "Transacting","Non Transacting", "Out Of Reach"]'
                                        data-values="[{{@$registered_users->count()}}, {{@$activated_users->count() . ','. @$active_users->count() . ','. @$non_transacting_users->count() . ','. @$users_out_of_reach->count()}}]"
                                        data-p-values="[{{number_format(@$register_user_percentage, 2)}}, {{number_format(@$connected_user_percentage, 2)}}, {{number_format(@$transacting_user_percentage, 2)}}, {{number_format(@$non_transacting_user_percentage, 2)}}, {{number_format(@$outofreach_user_percentage, 2)}}]">
                                </canvas>
                            </div>
                        </div>
                    </div>
