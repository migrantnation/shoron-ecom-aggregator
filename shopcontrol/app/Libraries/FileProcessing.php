...
class FileProcessing
{
    ...
    {
        if (file_exists($data['folder_old_path'])) {
            rename($data['folder_old_path'], $data['folder_path']);
        } else {
    ...
    public function create_folder($data)
    {
        if (!file_exists($data['folder_path'])) {
            mkdir($data['folder_path'], 0777, true);
        }
    }
    ...
    public function remove_folder($dir)
    {
        if (is_dir($dir)) {
            ...
            foreach ($files as $file) {
                (is_dir("$dir/$file")) ? $this->remove_user_folder("$dir/$file") : unlink("$dir/$file");
            }
            return rmdir($dir);
        }
    }
    ...
    public function convert_image_to_round($original_path, $dest_path)
    {
        $size = getimagesize($original_path);
        $w = $size[0];
        $h = $size[1]; // original size
